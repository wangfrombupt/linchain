...
def main():
    ...
    # state_dict = torch.load("/home/zc/data/moslora/MoSLoRA/triple_commonsense_reasoning/rte/model/base/pytorch_model.bin")
    # state_dict = {"base_model.model." + key : value for key, value in state_dict.items()}
    ...
    if len(trainable_params) > 0:
        for name, param in model.named_parameters():
            if name.startswith('base_model.model.deberta') or name.startswith('base_model.model.roberta'):
    ...
    if training_args.do_train:
        ...
        trainer.save_state()
        print("Complete trainer save...")

        model.save_pretrained(os.path.join(training_args.output_dir, "adapter"))
        model.base_model.save_pretrained(os.path.join(training_args.output_dir, "base"), safe_serialization=False)
        tokenizer.save_pretrained(os.path.join(training_args.output_dir, "base"))
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            for i in range(len(test_dataset)):
                ...
                input_ids = torch.tensor(test_dataset[i]['input_ids']).unsqueeze(0).to(model.device)
                attention_mask = torch.tensor(test_dataset[i]['attention_mask']).unsqueeze(0).to(model.device)

                # 使用模型进行推理
                with torch.no_grad():
                    outputs = model(input_ids, attention_mask=attention_mask)
                
                # 获取 logits 并选择最大值对应的标签

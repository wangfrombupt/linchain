...
class LoraConfig(PeftConfig):
    ...
    mixer_num: int = field(default=1, metadata={"help": "mixer num"})
    ...
    **Attributes**:
    ...
    def _find_and_replace(self):
        ...
        kwargs = {
            ...
            "mixer_num": self.peft_config.mixer_num,
        }
...
class Linear(nn.Linear, LoraLayer):
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            if self.lora_use_mixer:
                self.lora_mixer = nn.ModuleList([nn.Linear(r, r) for _ in range(self.mixer_num)])
        ...
        if hasattr(self, "lora_A"):
            ...
            if self.lora_use_mixer:
                for i in range(self.mixer_num):
                    nn.init.kaiming_uniform_(self.lora_mixer[i].weight, a=math.sqrt(5))
    ...
    def get_weight_mixer(self, index):
        if index == 0:
            return self.lora_mixer[index]
        else:
            return self.lora_mixer[index] @ self.get_weight_mixer(index - 1)
    ...
    def get_weight_A(self):
        ...
        if self.lora_use_mixer:
            return self.get_weight_mixer(self.mixer_num - 1) @ self.lora_A.weight
    ...
    def forward_mixer(self, index, x):
        if index == self.mixer_num - 1:
            return self.lora_mixer[index](x)
        else:
            return self.forward_mixer(index + 1, self.lora_mixer[index](x))
    ...
    def forward_A(self, x):
        ...
        if self.lora_use_mixer:
            return self.forward_mixer(0, self.lora_A(x))
